...
export async function getMovieListGroupByDate(type = 'now') {
...
export async function getTheaterList(cityId) {
...
export async function getMovieTimes(movieId, cityId) {
...
export async function getTheaterTimes(theaterId, cityId, date) {
    ...
    const $ = cheerio.load(htmlString);
    let ldJson = lruCache.get(`theater-info-${theaterId}`);
    if (!ldJson) {
        if (isToday) {
...
export const searchMovieRating = async (keyword, searchType = 'All') => {
...
export const searchMovieRatingDetails = async (certificateNumber) => {
...
export async function getVieShowNowMovieList(p = 1, maxPage = 0) {
...
export async function getVieShowComingMovieList(p = 1, maxPage = 0) {
    if (maxPage > 0 && p > maxPage) {
        return [];
    }
    const baseURL = `https://www.vscinemas.com.tw/vsweb/film`;
...
export async function getVieShowMovieShowTimes(cinemaCode) {
